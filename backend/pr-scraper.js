...
const DIST_OUTPUT_PATH = path.join(
  __dirname,
  '..',
  'frontend',
  'public',
...
const BASE_API_URL =
  'https://result.election.gov.np/Handlers/SecureJson.ashx?file=JSONFiles/Election2082/HOR/PR/HOR';
  ...
  for (const item of constsList) {
    ...
    for (let c = 1; c <= numConsts; c++) {
      ...
      if (data && Array.isArray(data) && data.length > 0) {
        console.log(
          `[SUCCESS] PR HOR-${distId}-${c}: ${data.length} parties | top: ${data[0].PoliticalPartyName}`,
        );
      ...
      const natSorted = Array.from(partyTotals.values()).sort(
...
async function main() {
  try {
    ...
    if (count > 0) {
      ...
      await runGit(
        'git add ../frontend/public/data/pr-results.json ../frontend/public/data/pr-by-district.json',
      );
      ...
      if (status.includes('pr-results.json') || status.includes('pr-by-district.json')) {
        try {
          await runGit(
            'git commit --amend --no-edit --no-verify ../frontend/public/data/pr-results.json ../frontend/public/data/pr-by-district.json',
          );
        } catch {
