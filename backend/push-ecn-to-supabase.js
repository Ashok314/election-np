...
async function scrapeAndPush() {
    ...
    for (let c = 1; c <= numConsts; c++) {
      ...
      if (data && Array.isArray(data)) {
        ...
        data.forEach((row) => {
          ...
          if (
            !uniqueCandidates.has(name) ||
            votes > (uniqueCandidates.get(name).TotalVoteReceived || 0)
          ) {
        ...
        const { error } = await supabase.from('election_results').upsert(rowsToInsert, {
  ...
  console.log(
    `[${new Date().toISOString()}] Scrape finished. Upserted ${totalUpserted} candidates to Supabase.`,
  );
  await browser.close();
}
...
async function executeWithRetry(maxRetries = 3) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      await scrapeAndPush();
      console.log(
        `[${new Date().toISOString()}] Scraper completed successfully on attempt ${attempt}.`,
      );
